...
public class Main extends SimpleApplication {
    ...
    private float enemy_distance = 100;
    ...
    private void initBot() {
        ...
        for (int i = 0; i < number_of_enemies; i++) {
            Node bot = createBall(new Vector3f(enemy_distance * (2 * (float) Math.random() - 1), 20, enemy_distance * (2 * (float) Math.random() - 1)));
            rootNode.attachChild(bot);
            bulletAppState.getPhysicsSpace().addAll(bot);
        }
    }

    private Node createBall(Vector3f location) {

        Node node = new Node("Evil ball");
        Geometry ball_geo = new Geometry("Evil ball geometry", sphere);
        Material mat = new Material(assetManager, "Common/MatDefs/Misc/Unshaded.j3md");
    ...
    public void simpleUpdate(float tpf) {
        ...
        for (Spatial sp : rootNode.getChildren()) {
            if ("Evil ball".equals(sp.getName())) {
                sp.getControl(RigidBodyControl.class).applyCentralForce(
                        (torso.getControl(RigidBodyControl.class).getPhysicsLocation().subtract(
                                sp.getControl(RigidBodyControl.class).getPhysicsLocation())).normalize().mult(100000f)
                );
            }
        }
    }
